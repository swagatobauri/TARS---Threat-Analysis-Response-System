...
import {
  ...
  Activity, Globe, Shield, ArrowLeft
} from "lucide-react";
...
export default function WarGamesPage() {
  ...
  const log = useCallback((msg: string) => {
    setConsoleLogs(p => [...p, `${format(new Date(), "HH:mm:ss.SSS")}  ${msg}`].slice(-40));
  }, []);
  ...
  const tick = useCallback(() => {
    ...
    // Attack traffic
    const doAttack = mode === "attack_only" || (mode === "mixed" && Math.random() < 0.3);
    if (doAttack) {
      ...
      if (aType === "brute_force") {
        ...
        for (let i = 0; i < count; i++) {
          ...
          batch.push({
            id: crypto.randomUUID(), timestamp: new Date().toISOString(),
            source_ip: attackerIp, dest, port: 22, anomaly_score: sc,
            risk_level: riskOf(sc), action: actionOf(riskOf(sc)),
        ...
        for (let i = 0; i < count; i++) {
          ...
          batch.push({
            ...
            dest, port: 80, anomaly_score: sc, risk_level: riskOf(sc),
            action: actionOf(riskOf(sc)), attack_type: "ddos",
          });
        }
        ...
        for (let i = 0; i < count; i++) {
          ...
          batch.push({
            id: crypto.randomUUID(), timestamp: new Date().toISOString(),
            source_ip: attackerIp, dest, port: base + i, anomaly_score: sc,
            risk_level: riskOf(sc), action: actionOf(riskOf(sc)),
      ...
      const avgScore = batch.filter(e => e.attack_type !== "normal")
        .reduce((a, e) => a + e.anomaly_score, 0) / Math.max(count, 1);
      log(`[THREAT] ${aType.toUpperCase()} → ${dest} | ${count} payloads | avg_score: ${avgScore.toFixed(3)}`);
    }

    setTotalLogs(p => p + batch.length);
    setThreats(p => [...p, ...batch].slice(-150));
    ...
    <div className="h-screen flex flex-col bg-black text-white overflow-hidden selection:bg-red-600">
      ...
      <div className="flex-1 grid grid-cols-12 gap-0 min-h-0 overflow-hidden">
        ...
        <div className="col-span-3 border-r border-[#1a1a1a] p-5 flex flex-col bg-[#050505] overflow-y-auto">
          {/* Target */}
          <div className="mb-5">
            <label className="text-[9px] font-mono tracking-widest text-[#555] uppercase block mb-2">
            ...
            <input
              ...
              placeholder="e.g. target-site.com"
              disabled={isRunning}
            ...
            <div className="grid grid-cols-3 gap-1">
              {["normal", "mixed", "attack_only"].map(m => (
                <button key={m} onClick={() => setMode(m)} disabled={isRunning}
                  className={`py-1.5 text-[8px] font-mono tracking-widest uppercase border transition-colors ${
                    mode === m ? "bg-[#111] border-[#cc0000] text-[#cc0000]" : "bg-transparent border-[#1a1a1a] text-[#555]"
                  }`}
                >{m.replace("_", " ")}</button>
              ))}
            </div>
            ...
            <div className="space-y-1">
              {[
                ...
                <button key={a.id} onClick={() => setAttackType(a.id)} disabled={isRunning}
                  className={`w-full py-2.5 px-3 flex items-center gap-3 text-left border transition-colors ${
                    attackType === a.id ? "bg-[#1a0505] border-[#cc0000]" : "bg-transparent border-[#1a1a1a]"
                  }`}
                >
                  <a.icon size={14} className={attackType === a.id ? "text-[#cc0000]" : "text-[#444]"} />
                  <div>
                    <div className={`text-[10px] font-mono tracking-widest uppercase ${attackType === a.id ? "text-[#cc0000]" : "text-[#666]"}`}>
                      {a.label}
                    </div>
          ...
          <button onClick={toggle}
            className={`mt-auto w-full py-4 flex items-center justify-center gap-2 text-xs font-mono tracking-widest uppercase font-bold border-2 transition-all ${
              isRunning
                ? "border-[#cc0000] text-[#cc0000] bg-[#0a0505] hover:bg-[#1a0505]"
                : "border-[#cc0000] bg-[#cc0000] text-black hover:bg-[#ff0000]"
            }`}
          >
        ...
        <div className="col-span-9 flex flex-col min-h-0 overflow-hidden">
          ...
          {/* Console */}
          <div className="h-32 border-b border-[#1a1a1a] bg-black p-3 overflow-y-auto flex-shrink-0 font-mono text-[10px] leading-relaxed">
            {consoleLogs.map((l, i) => (
              <div key={i} className={l.includes("[THREAT]") ? "text-[#cc0000]" : l.includes("[ERROR]") ? "text-[#ff4444]" : "text-[#00ff88]"}>
                {l}
              </div>
            ))}
          ...
          <div className="flex-1 overflow-auto bg-[#050505]">
              ...
              <tbody>
                {[...threats].reverse().slice(0, 80).map(t => (
                  <tr key={t.id} className={`border-b border-[#111] ${t.risk_level === "CRITICAL" ? "bg-[#0a0303]" : ""}`}>
                    ...
                    <td className="px-3 py-1.5 text-[#888]">{t.attack_type}</td>
                    ...
                    <td className={`px-3 py-1.5 ${t.action === "BLOCK_IP" ? "text-[#cc0000] font-bold" : t.action === "RATE_LIMIT" ? "text-[#ffaa00]" : "text-[#555]"}`}>
                      {t.action}
                    </td>
                  </tr>
                ))}
            ...
            {threats.length === 0 && (
              <div className="p-12 text-center font-mono text-[#333] text-xs tracking-widest">
                _ AWAITING TARGET ENGAGEMENT
              </div>
            )}
          </div>
        </div>
